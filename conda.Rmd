...
This is where it gets complicated and where the situation is different for `R` and `python`. We could simply install packages from other sources (github, CRAN) but `anaconda` will not track them and thus the environment will not be reproducible. 
...
This includes all packages from github and self-developed packages. Your `anaconda` environment should already contain the OS-specific compilers.
